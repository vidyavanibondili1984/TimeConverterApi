...
public class TimeConverterServiceImpl implements TimeConverterService{
	...
	public String convertTimeToString(String hourAndMinute) {
		...
		//String localTimeString=dateTimeFormatter.format(hourAndMinute);
		
		
		String split_hours_minutes[]=hourAndMinute.split(":",2);
		//String split_hours_minutes[]=localTimeString.split(":",2);
		...
		if((hours>=1&&hours<=24)&&(minutes>=0&&minutes<=59)) {
			...
			System.out.println("Unexpected error while converting time to string format "+e);
			
			throw new TechnicalException("UnexpectedError");
